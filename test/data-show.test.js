...
test('separate documents each receive their own visibility style', () => {
  ...
  const firstCleanup = render(firstFragment, { store: createStore({ visible: false }), document: first.window.document });
  const secondCleanup = render(secondFragment, { store: createStore({ visible: false }), document: second.window.document });
